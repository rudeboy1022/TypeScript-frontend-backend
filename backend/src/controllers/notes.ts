...
import { assertIdDefined } from "../utils/assertIsDefined";

export const getSample: RequestHandler = async (req, res, next) => {
  try {
    const sample = [
      { id: 1, value: "a" },
      { id: 2, value: "b" },
    ...
    res.status(200).json(JSON.stringify(sample));
  } catch (error) {
    next(error);
  }
};
...
export const getNotes: RequestHandler = async (req, res, next) => {
  ...
  try {
    assertIdDefined(authenticateUserId);
